/**
 * Inchworm — happy path.
 * Calibrates side-on (instant confirm ~200ms), then performs clean forward folds.
 *
 * Rep cycle (3000ms each):
 *   0–1000 ms: hinge 0° → 65°
 *   1000–1500: hold at bottom (65°)
 *   1500–2500: hinge 65° → 0°
 *   2500–3000: stand still
 */
import { describe, it, expect } from 'vitest';
import { buildFrames } from '../../harness/frame-stream';
import { buildInchwormPose } from '../../harness/pose-stub';
import { runInchwormSession, warningsOtherThan } from '../../harness/runner';
...
function happyPathIntent(reps: number) {
  ...
  return {
    totalMs,
    intentAt: (tMs: number): InchwormPoseIntent => {
      if (tMs < calMs) {
...
describe('Inchworm — happy path', () => {
  it('calibrates within 500ms and counts 4 reps', () => {
    const { totalMs, intentAt } = happyPathIntent(4);
    const frames = buildFrames(intentAt, buildInchwormPose, { fps: 30, durationMs: totalMs });
  ...
  it('rep depthDeg reflects peak hinge angle (EMA-smoothed)', () => {
    const { totalMs, intentAt } = happyPathIntent(2);
    const frames = buildFrames(intentAt, buildInchwormPose, { fps: 30, durationMs: totalMs });
    const result = runInchwormSession(frames);
    for (const rep of result.completedReps) {
      // EMA-attenuated from 65° target — expect >= 45° with alpha=0.15
