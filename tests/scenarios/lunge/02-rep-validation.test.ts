...
describe('Lunge — rep validation gates', () => {
  ...
  it('rejects bilateral-squat imposters (both legs bend equally)', () => {
    // Both legs flex to 90° identically — that's a squat, not a lunge.
    // Engine should reject via the front-back gap gate (gap < 20°).
    const frames = makeFrames(
      (t) => {
        let flex: number;
  ...
  it('accepts a valid rep at the minimum-depth boundary (55° front + back straight)', () => {
    const frames = makeFrames(
      (t) => {
        let flex: number;
