...
import { runBroadJumpSession, warningsOtherThan } from '../../harness/runner';
...
function happyPathIntent(reps: number) {
  ...
  return {
    totalMs,
    intentAt: (tMs: number): BroadJumpPoseIntent => {
      if (tMs < calMs) {
...
describe('Broad Jump — happy path', () => {
  it('calibrates within 500ms', () => {
    const { totalMs, intentAt } = happyPathIntent(1);
    const frames = buildFrames(intentAt, buildBroadJumpPose, { fps: 30, durationMs: totalMs });
