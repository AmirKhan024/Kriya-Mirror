/**
 * Regression test for round-7 Walking Lunge fix: idle `not-moving` warning
 * must fire after a real step, not just from cold-start STANDING. The bug:
 * user did 2 steps, then idled for 17 seconds, no `not-moving` ever fired.
 * Root cause was the post-rep EMA-decay tail (smoothedFlexion drifting from
 * ~17° to 0° over several seconds) permanently inflating max - min, so
...
describe('Walking Lunge — regression: not-moving fires after a real step + idle (2026-05-25)', () => {
  it('DOES fire not-moving when user rests in STANDING after completing a step', () => {
    // Profile: stand-still during calibration → one full walking lunge step
    // (0 → 90 → 0 over 2.5s) → 8s of STANDING idle.
    ...
    const frames = buildFrames(
      (tMs) => {
        ...
        if (tMs < REP_END_MS) {
          // Real step: 0 → 90 over 1s, hold 90 for 0.5s, 90 → 0 over 1s.
    ...
    // The whole point: idle warning must fire post-step.
