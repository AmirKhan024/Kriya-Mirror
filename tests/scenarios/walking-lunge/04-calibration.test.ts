...
import { runWalkingLungeSession } from '../../harness/runner';
import { IDX } from '../../harness/types';

describe('Walking Lunge — calibration gates', () => {
  ...
  it('fails when arms are overhead (armsAtSides gate)', () => {
    // Wrists above shoulders fails the "arms at sides" check that walking lunge needs.
    const frames = buildFrames(
  ...
  it('fails when feet are too wide (feetTogether gate)', () => {
    // Feet at 1.5× shoulder width fails the walking lunge feet-together check.
    const frames = buildFrames(
