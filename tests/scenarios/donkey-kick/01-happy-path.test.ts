/**
 * Donkey Kick — happy-path tests.
 * Verifies: calibration confirms, 4 clean kicks count as reps, mqs > 70, no warnings.
 */
import { describe, it, expect } from 'vitest';
import type { PoseLandmarks } from '@/modules/pose/types';
import type { CalibrationUpdate } from '@/modules/squat/types';
import { DonkeyKickEngine } from '@/modules/donkey-kick/engine';
import type { DonkeyKickRepEvent, DonkeyKickFrameMetrics } from '@/modules/donkey-kick/types';
...
// thighLiftDeg: 0 = at rest (knee below hip), 65+ = kick near top
// ---------------------------------------------------------------------------
function buildDKPoseLocal(intent: {
  thighLiftDeg: number;
  bodySpan?: number;
  visibility?: number;
}): PoseLandmarks {
  ...
  // Side camera geometry (user facing right)
  const SHOULDER_X = 0.68;
  const SHOULDER_Y = 0.42;
  const HIP_X = 0.45;
  const HIP_Y = 0.42;   // same height = body horizontal for calibration
  const L_THIGH = 0.18;
  const L_SHIN = 0.18;
  ...
  // Shin points backward (left) — gives body span > 0.40
  ...
  // Left side (dominant — near side to camera)
  lm[11] = { x: SHOULDER_X, y: SHOULDER_Y, z: 0, visibility: vis }; // left shoulder
  lm[12] = { x: SHOULDER_X, y: SHOULDER_Y + 0.01, z: 0, visibility: vis * 0.7 }; // right shoulder
  lm[23] = { x: HIP_X, y: HIP_Y, z: 0, visibility: vis };           // left hip
  lm[24] = { x: HIP_X, y: HIP_Y + 0.01, z: 0, visibility: vis * 0.7 }; // right hip
  lm[25] = { x: kneeX, y: kneeY, z: 0, visibility: vis };           // left knee (active)
  lm[26] = { x: HIP_X, y: HIP_Y + L_THIGH, z: 0, visibility: vis * 0.6 }; // right knee (rest)
  lm[27] = { x: ankleX, y: ankleY, z: 0, visibility: vis };         // left ankle
  lm[28] = { x: HIP_X - L_SHIN, y: HIP_Y + L_THIGH + L_SHIN*0.3, z: 0, visibility: vis * 0.6 }; // right ankle
  lm[15] = { x: wristX, y: wristY, z: 0, visibility: vis };         // left wrist
  lm[16] = { x: wristX - 0.35, y: wristY, z: 0, visibility: vis * 0.7 }; // right wrist
...
// ---------------------------------------------------------------------------
// Rep cycle helper: calibrate 2.2s, then rep cycle at given peak
// ---------------------------------------------------------------------------
function happyPathIntent(reps: number, thighLiftPeakDeg = 65) {
  const calMs = 2200;
...
// Tests
// ---------------------------------------------------------------------------
describe('Donkey Kick — happy path', () => {
  it('calibrates within 2300ms and counts 4 clean kicks', () => {
    const { totalMs, intentAt } = happyPathIntent(4);
    const frames = buildFrames(intentAt, buildDKPoseLocal, { fps: 30, durationMs: totalMs });
  ...
  it('no incomplete-donkey-kick warnings on clean full-lift reps', () => {
    const { totalMs, intentAt } = happyPathIntent(3, 65);
    const frames = buildFrames(intentAt, buildDKPoseLocal, { fps: 30, durationMs: totalMs });
    const result = runDKLocal(frames);
  ...
  it('peak thighLiftDeg is recorded in depthDeg field', () => {
    const { totalMs, intentAt } = happyPathIntent(2, 65);
    const frames = buildFrames(intentAt, buildDKPoseLocal, { fps: 30, durationMs: totalMs });
    const result = runDKLocal(frames);
    expect(result.completedReps.length).toBeGreaterThanOrEqual(2);
    result.completedReps.forEach((rep) => {
      // depthDeg should be close to the peak (at least 45° for a valid rep)
