/**
 * Warrior I's knee/trunk warnings. All recoverable per Fix S — they freeze the
 * timer, fire a warning, but do NOT terminate the workout. (Arms-overhead has
 * its own dedicated test file, 08.)
 */
...
describe('Warrior I — knee + trunk form warnings (Fix S recoverable)', () => {
  ...
  it('does NOT fire any knee/trunk warning on clean continuous form (sanity)', () => {
    const frames = buildFrames(
      () => ({ frontKneeFlexionDeg: 90, backKneeFlexionDeg: 5, trunkLeanDeg: 5 } as WarriorOnePoseIntent),
      buildWarriorOnePose,
      { fps: 30, durationMs: HOLD_START + 8000 },
    );
    const result = runWarriorOneSession(frames);
  ...
  it('accepts user holding at 60° front-knee flex without warnings', () => {
    const frames = buildFrames(
      () => ({ frontKneeFlexionDeg: 60, backKneeFlexionDeg: 5 } as WarriorOnePoseIntent),
      buildWarriorOnePose,
