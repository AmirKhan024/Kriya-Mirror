...
describe('Warrior I — position-lost warning (Fix N)', () => {
  it('fires position-lost after 3+ seconds of null landmarks post-calibration', () => {
    const frames = buildFrames(
      (tMs) => {
        if (tMs < CAL_MS) return { frontKneeFlexionDeg: 90 } as WarriorOnePoseIntent;
        return null;
      },
      buildWarriorOnePose,
  ...
  it('does NOT fire position-lost on a clean continuous stream', () => {
    const frames = buildFrames(
      () => ({ frontKneeFlexionDeg: 90 } as WarriorOnePoseIntent),
      buildWarriorOnePose,
  ...
  it('does NOT fire position-lost during the brief calibration phase', () => {
    const frames = buildFrames(
      (tMs) => {
        ...
        return { frontKneeFlexionDeg: 90 } as WarriorOnePoseIntent;
      },
      buildWarriorOnePose,
