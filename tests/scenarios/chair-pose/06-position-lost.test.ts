 ...
 * 10s while still lost. Mirrors the plank / tandem-stand / SLS pattern.
 */
...
describe('Chair Pose — position-lost warning (Fix N)', () => {
  it('fires position-lost after 3+ seconds of null landmarks post-calibration', () => {
    const frames = buildFrames(
      (tMs) => {
        if (tMs < CAL_MS) {
          return { kneeFlexionDeg: 90, side: 'left' as const } as ChairPosePoseIntent;
        }
        return null;
      },
      buildChairPosePose,
  ...
  it('does NOT fire position-lost during the brief calibration phase', () => {
    // Null frames before cal-confirm should not trigger the post-cal heartbeat.
    const frames = buildFrames(
      (tMs) => {
