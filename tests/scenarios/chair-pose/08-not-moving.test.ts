/**
 * Round 20 — `not-moving` idle prompt for chair-pose.
 *
 * Fires when form has been broken for ≥ 5 s; repeats every 15 s.
 */
import { describe, it, expect } from 'vitest';
import { buildFrames } from '../../harness/frame-stream';
import { buildChairPosePose } from '../../harness/pose-stub';
import { runChairPoseSession, countWarnings } from '../../harness/runner';
import type { ChairPosePoseIntent } from '../../harness/types';

const CAL_MS = 2200;
const HOLD_START = CAL_MS;

describe('Chair Pose — not-moving idle prompt (round 20)', () => {
  it('fires not-moving after 5 s of sustained form-break (knees straight)', () => {
    const frames = buildFrames(
      (tMs): ChairPosePoseIntent => {
        const intoHold = tMs - HOLD_START;
        // Cal-confirm with 90° flex, then knees straighten (form-break).
        const kneeFlex = intoHold < 1000 ? 90 : 25;
        return { kneeFlexionDeg: kneeFlex, side: 'left' };
      },
      buildChairPosePose,
      { fps: 30, durationMs: HOLD_START + 10_000 },
    );
    const result = runChairPoseSession(frames);
    expect(result.finalCalibration?.state).toBe('confirmed');
    expect(countWarnings(result, 'not-moving')).toBeGreaterThan(0);
  });

  it('repeats not-moving every ~15 s while still broken', () => {
    const frames = buildFrames(
      (tMs): ChairPosePoseIntent => {
        const intoHold = tMs - HOLD_START;
        const kneeFlex = intoHold < 1000 ? 90 : 25;
        return { kneeFlexionDeg: kneeFlex, side: 'left' };
      },
      buildChairPosePose,
      { fps: 30, durationMs: HOLD_START + 25_000 },
    );
    const result = runChairPoseSession(frames);
    expect(countWarnings(result, 'not-moving')).toBeGreaterThanOrEqual(2);
  });

  it('does NOT fire not-moving on a clean hold', () => {
    const frames = buildFrames(
      (): ChairPosePoseIntent => ({ kneeFlexionDeg: 90, side: 'left' }),
      buildChairPosePose,
      { fps: 30, durationMs: HOLD_START + 10_000 },
    );
    const result = runChairPoseSession(frames);
    expect(countWarnings(result, 'not-moving')).toBe(0);
  });

  it('does NOT fire not-moving when form recovers within 5 s', () => {
    const frames = buildFrames(
      (tMs): ChairPosePoseIntent => {
        const intoHold = tMs - HOLD_START;
        // Brief 3 s form-break window (knees straight), then back to 90°.
        const kneeFlex = (intoHold >= 1000 && intoHold < 4000) ? 25 : 90;
        return { kneeFlexionDeg: kneeFlex, side: 'left' };
      },
      buildChairPosePose,
      { fps: 30, durationMs: HOLD_START + 10_000 },
    );
    const result = runChairPoseSession(frames);
    expect(countWarnings(result, 'not-moving')).toBe(0);
  });
});
