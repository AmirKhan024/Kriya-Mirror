...
describe('Warrior II — bad-form time is discarded (Fix B + Fix V)', () => {
  it('freezes the hold counter during sustained front-knee-not-bent-enough', () => {
    const frames = buildFrames(
      (tMs): WarriorTwoPoseIntent => {
        ...
        return { frontKneeFlexionDeg: 35 };  // round-19: was 50, now below the new 50° threshold
      },
      buildWarriorTwoPose,
  ...
  it('does NOT freeze on single-frame jitter (Fix V hysteresis)', () => {
    const frames = buildFrames(
      (tMs): WarriorTwoPoseIntent => {
        ...
        return { frontKneeFlexionDeg: isBadFrame ? 50 : 90 };
      },
      buildWarriorTwoPose,
