/**
 * JumpSquatEngine — rep-based tracker for front-camera Jump Squat.
 *
 * State machine:
 ...
 *   - incomplete-jump-squat — max hip rise < MIN_HIP_RISE
...
import { JumpSquatCalibration } from './calibration';
import type { JumpSquatBaseline, JumpSquatEngineCallbacks, JumpSquatFrameMetrics, JumpSquatRepState } from './types';
...
const MIN_REP_DURATION_MS = 600;      // full jump squat cycle
...
export class JumpSquatEngine {
  private callbacks: JumpSquatEngineCallbacks;
  private calibration: JumpSquatCalibration;
  private baseline: JumpSquatBaseline | null = null;

  private repState: JumpSquatRepState = 'STANDING';
  ...
  constructor(callbacks: JumpSquatEngineCallbacks = {}) {
    this.callbacks = callbacks;
    this.calibration = new JumpSquatCalibration();
  }
  ...
  update(landmarks: PoseLandmarks | null, now: number): void {
    ...
    if (!this.calibration.isConfirmed()) {
      ...
      if (calUpdate.state === 'confirmed') {
        ...
        if (this.baseline) {
          debugLog('JUMPSQUAT', 'CALIB', 'CONFIRMED', {
  ...
  private processTrackingFrame(landmarks: PoseLandmarks, now: number): void {
    ...
    const metrics: JumpSquatFrameMetrics = {
      hipY: rawHipY,
    ...
    switch (this.repState) {
      case 'STANDING':
        if (hipDisp > LOAD_ENTER_THRESHOLD) {
          ...
          debugLog('JUMPSQUAT', 'STATE', 'STANDING → LOADING', {
      ...
      case 'LOADING':
        if (hipVelocity < -JUMP_VELOCITY_THRESHOLD) {
          ...
          debugLog('JUMPSQUAT', 'STATE', 'LOADING → AIRBORNE', {
            velocity: +hipVelocity.toFixed(3),
          });
        }
        if (hipDisp < -LOAD_ENTER_THRESHOLD) {
          this.repState = 'AIRBORNE';
          debugLog('JUMPSQUAT', 'STATE', 'LOADING → AIRBORNE (pos)', {
            hipDisp: +hipDisp.toFixed(3),
          });
        }
        break;
      ...
      case 'AIRBORNE':
        if (hipVelocity > LANDING_VELOCITY_THRESHOLD) {
          ...
          debugLog('JUMPSQUAT', 'STATE', 'AIRBORNE → LANDING', {
            velocity: +hipVelocity.toFixed(3),
      ...
      case 'LANDING':
        if (kneeAngle > ABSORB_KNEE_THRESHOLD) {
          ...
          debugLog('JUMPSQUAT', 'STATE', 'LANDING → ABSORBING', {
  ...
  private validateRepShape(now: number): {
    ...
    // incomplete-jump-squat before too-fast: if the jump was tiny, "not high enough" is
    // more actionable feedback than "too fast". Mirrors Fix D from bilal_prompt.md.
    if (this.maxHipRiseThisRep < MIN_HIP_RISE) {
      return { ok: false, reason: 'incomplete-jump-squat', warnings: [] };
    }
  ...
  private completeRep(now: number): void {
    ...
    if (!validation.ok) {
      debugLog('JUMPSQUAT', 'REJECT', 'Rep discarded', {
        reason: validation.reason,
        maxRise: +maxRise.toFixed(3),
        durationMs,
        didLoad,
      });
      if (validation.reason === 'incomplete-jump-squat') {
        this.maybeEmitWarning('incomplete-jump-squat', true, now);
      } else {
    ...
    debugLog('JUMPSQUAT', 'REP', 'Rep complete', {
      ...repPayload,
      durationMs,
      didLoad,
      maxRise: +maxRise.toFixed(3),
  ...
  private checkNoMovement(now: number): void {
    ...
    if (
      ...
      debugLog('JUMPSQUAT', 'WARN', 'not-moving', {
  ...
  private checkPositionLost(haveValidFrame: boolean, now: number): void {
    ...
    debugLog('JUMPSQUAT', 'WARN', 'position-lost', { lostMs: Math.round(lostMs) });
