...
export interface WallSitBaseline {
  ...
  /** Trunk lean at calibration in degrees from vertical (0 = upright back, flat on wall). */
  initialTrunkLeanDeg: number;
}

export interface WallSitFrameMetrics {
  /** Current EMA-smoothed knee flexion (0 = standing, 90 = thighs parallel). */
  kneeFlexionDeg: number;
  /** Current EMA-smoothed trunk lean from vertical (0 = upright, positive = leaning off the wall). */
  trunkLeanDeg: number;
  /** Heel lift amount: max(0, baseline.ankleY − currentAnkleY) in normalized coords. */
  heelLiftAmount: number;
  /** Shoulder rise vs baseline: positive value means user is sliding/standing back up. */
  shoulderRise: number;
...
export interface WallSitEngineCallbacks {
  onCalibrationUpdate?: (update: CalibrationUpdate) => void;
  /** Tick payload mirrors chair-pose: `longestUnfrozenSec` carries the longest
   *  continuous valid streak (Fix U) for the report. */
  onHoldTick?: (tick: {
  ...
  onFrame?: (metrics: WallSitFrameMetrics) => void;
}
