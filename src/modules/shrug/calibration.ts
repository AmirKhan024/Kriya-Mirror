/**
 * Shrug calibration — 4 gates mirroring hammer-curl's structure:
...
import type { ShrugBaseline } from './types';
...
export class ShrugCalibration {
  private startedAt: number;
  private goodPostureStart = 0;
  private badPostureStart = 0;
  private confirmedBaseline: ShrugBaseline | null = null;
  ...
  update(landmarks: PoseLandmarks | null, now: number): CalibrationUpdate {
    ...
    if (prevState !== this.state) {
      debugLog('SHRUG', 'CALIB', `${prevState} → ${this.state}`, {
        gates: {
  ...
  private checkGates(landmarks: PoseLandmarks): {
    ...
    baselineCandidate: ShrugBaseline | null;
  } {
  ...
  getBaseline(): ShrugBaseline | null { return this.confirmedBaseline; }
}
