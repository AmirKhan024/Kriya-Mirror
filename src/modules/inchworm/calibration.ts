/**
 * Inchworm calibration — side camera, person standing upright with arms relaxed
 * at sides.
 *
...
import type { InchwormBaseline } from './types';
...
export class InchwormCalibration {
  private startedAt: number;
  private goodPostureStart = 0;
  private badPostureStart = 0;
  private confirmedBaseline: InchwormBaseline | null = null;
  ...
  update(landmarks: PoseLandmarks | null, now: number): CalibrationUpdate {
    ...
    if (prevState !== this.state) {
      debugLog('INCHWORM', 'CALIB', `${prevState} → ${this.state}`, {
        gates: {
  ...
  private checkGates(landmarks: PoseLandmarks): {
    ...
    baselineCandidate: InchwormBaseline | null;
  } {
    ...
    const baseline: InchwormBaseline = {
  ...
  getBaseline(): InchwormBaseline | null { return this.confirmedBaseline; }
}

/** Adapt InchwormBaseline to the shared CalibrationBaseline shape the play page reads. */
function toSquatBaseline(b: InchwormBaseline): CalibrationBaseline {
  return {
