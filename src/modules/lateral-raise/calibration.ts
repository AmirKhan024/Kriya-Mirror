/**
 * Lateral Raise calibration — 4 gates mirroring bicep-curl's structure:
 *   fullBodyVisible → shoulders+elbows+wrists+hips+ankles visible
 *   feetWide        → feetStable (feet within 1.20× shoulder width — no wide squat stance)
 *   armsOverhead    → armsAtSides (BOTH arms at < 25° abduction — hanging at sides, the DOWN start position)
 *   distanceOk      → body span in frame, with shoulderWidth floor (Fix X)
 *
 * Side bilateral: both arms must be at sides for cal to confirm.
 */
import type { PoseLandmarks } from '@/modules/pose/types';
import type { CalibrationState, CalibrationUpdate } from '@/modules/squat/types';
import { LM, lmVisible, midpoint, shoulderAbductionDeg } from './geometry';
import type { LateralRaiseBaseline } from './types';
import { debugLog } from '@/lib/debug';

// Fix G: instant calibration — confirms after ~6 frames of clean pose.
const CONFIRM_DURATION_MS = 200;
const BAD_POSTURE_BUFFER_MS = 300;
// Fix J: calibration timeout.
const TIMEOUT_MS = 20_000;

const MAX_FEET_RATIO = 1.20;            // feet ≤ 1.20× shoulder width
const ARMS_AT_SIDES_ABDUCTION_MAX = 25; // both arms < 25° abduction at calibration

const BODY_HEIGHT_MIN = 0.45;
const BODY_HEIGHT_MAX = 0.92;

// Fix X — narrow shoulderWidth at calibration → degenerate baseline (every
// distance-normalized threshold collapses to MediaPipe noise). Surface as
// 'too-far' so the user steps closer.
...
export class LateralRaiseCalibration {
  private startedAt: number;
  private goodPostureStart = 0;
  private badPostureStart = 0;
  private confirmedBaseline: LateralRaiseBaseline | null = null;
  ...
  update(landmarks: PoseLandmarks | null, now: number): CalibrationUpdate {
    ...
    if (prevState !== this.state) {
      debugLog('RAISE', 'CALIB', `${prevState} → ${this.state}`, {
        gates: {
  ...
  private checkGates(landmarks: PoseLandmarks): {
    ...
    baselineCandidate: LateralRaiseBaseline | null;
  } {
    ...
    // Arms at sides (low abduction) — the DOWN start position.
    const leftAbd = shoulderAbductionDeg(ls, lw, lh);
    const rightAbd = shoulderAbductionDeg(rs, rw, rh);
    const armsAtSides = leftAbd < ARMS_AT_SIDES_ABDUCTION_MAX && rightAbd < ARMS_AT_SIDES_ABDUCTION_MAX;
    ...
    if (bodyHeight < BODY_HEIGHT_MIN || shoulderWidth < MIN_SHOULDER_WIDTH) {
      // Fix X: shoulderWidth below floor → baseline would be degenerate.
      distanceOk = false;
    ...
    const checks = {
      ...
      feetWide: feetStable,         // remap: "wide" slot → "stable"
    ...
    const baseline: LateralRaiseBaseline = {
      shoulderMid,
      hipMid,
  ...
  getBaseline(): LateralRaiseBaseline | null { return this.confirmedBaseline; }
}
