/**
 * BroadJump calibration — front camera, person standing upright in jump stance.
 *
...
import type { BroadJumpBaseline } from './types';
...
export class BroadJumpCalibration {
  private startedAt = -1;
  private goodPostureStart = 0;
  private badPostureStart = 0;
  private confirmedBaseline: BroadJumpBaseline | null = null;
  ...
  update(landmarks: PoseLandmarks | null, now: number): CalibrationUpdate {
    ...
    if (prevState !== this.state) {
      debugLog('BROADJUMP', 'CALIB', `${prevState} → ${this.state}`, {
        gates: {
  ...
  private checkGates(landmarks: PoseLandmarks): {
    ...
    baselineCandidate: BroadJumpBaseline | null;
  } {
    ...
    const baseline: BroadJumpBaseline = {
      hipY: hipMid.y,
  ...
  getBaseline(): BroadJumpBaseline | null { return this.confirmedBaseline; }
}

/** Adapt BroadJumpBaseline to the shared CalibrationBaseline shape the play page reads. */
export function toSquatBaseline(b: BroadJumpBaseline): CalibrationBaseline {
  return {
