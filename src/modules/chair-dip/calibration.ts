/**
 * Chair Dip calibration — 4 gates mirroring squat's structure, remapped:
 ...
 *   armsOverhead    → armsExtended (BOTH elbows at flex < 30° — arms hanging ready at sides)
...
import type { ChairDipBaseline } from './types';
...
const ARMS_EXTENDED_FLEX_MAX = 30;     // both elbows < 30° flex at calibration (chair dip starting position allows slightly more flex)
...
export class ChairDipCalibration {
  private startedAt: number;
  private goodPostureStart = 0;
  private badPostureStart = 0;
  private confirmedBaseline: ChairDipBaseline | null = null;
  ...
  update(landmarks: PoseLandmarks | null, now: number): CalibrationUpdate {
    ...
    if (prevState !== this.state) {
      debugLog('CHAIR_DIP', 'CALIB', `${prevState} → ${this.state}`, {
        gates: {
  ...
  private checkGates(landmarks: PoseLandmarks): {
    ...
    baselineCandidate: ChairDipBaseline | null;
  } {
    ...
    // Arms extended (both elbows nearly straight) per BB1 spec
    ...
    const checks = {
      ...
      armsOverhead: armsExtended,   // remap: "overhead" → "extended at sides"
      distanceOk,
    };
    ...
    const baseline: ChairDipBaseline = {
      shoulderMid,
      hipMid,
  ...
  getBaseline(): ChairDipBaseline | null { return this.confirmedBaseline; }
}
