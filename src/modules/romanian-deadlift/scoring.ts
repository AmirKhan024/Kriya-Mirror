/**
 * Romanian Deadlift scoring.
 *
 * Completion keyed to peak hip-hinge angle (how deep the hinge reached).
 * RDL full stretch = ~60-70° (less than conventional DL because knees stay soft).
 * Smoothness from hip-Y velocity coefficient of variation.
...
export function getCompletionScore(peakHingeDeg: number): number {
  if (peakHingeDeg >= 70) return 100;
  if (peakHingeDeg >= 60) return 75;
  if (peakHingeDeg >= 50) return 50;
  if (peakHingeDeg >= 35) return 25;
  return 0;
}

/** Form sub-score from per-frame adherence counts (back straight, hip level). */
