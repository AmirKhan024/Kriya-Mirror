/**
 * Bird-Dog calibration — side camera, mirrors dead-bug's pattern.
 ...
 *   feetWide        → body horizontal (|dx|/|dy| >= 2.5) — shoulder and hip at same height
...
import type { BirdDogBaseline } from './types';
...
export class BirdDogCalibration {
  private startedAt: number;
  private goodPostureStart = 0;
  private badPostureStart = 0;
  private confirmedBaseline: BirdDogBaseline | null = null;
  ...
  update(landmarks: PoseLandmarks | null, now: number): CalibrationUpdate {
    ...
    if (prevState !== this.state) {
      debugLog('BIRD-DOG', 'CALIB', `${prevState} → ${this.state}`, {
        gates: {
  ...
  private checkGates(landmarks: PoseLandmarks): {
    ...
    baselineCandidate: BirdDogBaseline | null;
  } {
    ...
    const baseline: BirdDogBaseline = {
      side,
      bodyLength: dx,
      hipY: hip.y,
      kneeY: knee.y,
  ...
  getBaseline(): BirdDogBaseline | null { return this.confirmedBaseline; }
}
 ...
 */
function toSquatBaseline(b: BirdDogBaseline): CalibrationBaseline {
