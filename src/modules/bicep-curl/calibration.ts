/**
 * Bicep Curl calibration — 4 gates mirroring squat's structure, remapped:
...
import type { BicepCurlBaseline } from './types';
...
const ARMS_EXTENDED_FLEX_MAX = 25;     // both elbows < 25° flex at calibration
...
export class BicepCurlCalibration {
  private startedAt: number;
  private goodPostureStart = 0;
  private badPostureStart = 0;
  private confirmedBaseline: BicepCurlBaseline | null = null;
  ...
  update(landmarks: PoseLandmarks | null, now: number): CalibrationUpdate {
    ...
    if (prevState !== this.state) {
      debugLog('CURL', 'CALIB', `${prevState} → ${this.state}`, {
        gates: {
  ...
  private checkGates(landmarks: PoseLandmarks): {
    ...
    baselineCandidate: BicepCurlBaseline | null;
  } {
    ...
    const baseline: BicepCurlBaseline = {
      shoulderMid,
      hipMid,
  ...
  getBaseline(): BicepCurlBaseline | null { return this.confirmedBaseline; }
}
