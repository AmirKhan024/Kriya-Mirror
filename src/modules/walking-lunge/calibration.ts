/**
 * Walking Lunge calibration — front camera, mirrors lunge's 4-gate structure.
 * The walking lunge starting position is upright with feet roughly hip-width
 * and arms relaxed at sides. Identical to lunge calibration.
 *
...
import type { WalkingLungeBaseline } from './types';
...
// (walking lunges start hip-width; shoulder-wide stance is squat territory).
...
export class WalkingLungeCalibration {
  private startedAt: number;
  private goodPostureStart = 0;
  private badPostureStart = 0;
  private confirmedBaseline: WalkingLungeBaseline | null = null;
  ...
  update(landmarks: PoseLandmarks | null, now: number): CalibrationUpdate {
    ...
    if (prevState !== this.state) {
      debugLog('WALKING-LUNGE', 'CALIB', `${prevState} → ${this.state}`, {
        gates: {
  ...
  private captureBaseline(landmarks: PoseLandmarks): WalkingLungeBaseline {
    const ls = landmarks[LM.LEFT_SHOULDER];
    const rs = landmarks[LM.RIGHT_SHOULDER];
  ...
  getBaseline(): WalkingLungeBaseline | null { return this.confirmedBaseline; }
}
 ...
 */
function toSquatBaseline(b: WalkingLungeBaseline): CalibrationBaseline {
  return {
