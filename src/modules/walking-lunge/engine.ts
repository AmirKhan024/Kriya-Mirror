/**
 * WalkingLungeEngine — rep-based tracker for front-camera Walking Lunge.
 *
 * Mirrors LungeEngine's 4-state machine but operates UNILATERALLY: at the
 ...
 *   ASCENT_FROM_PEAK_DEG or 3°+ per frame) → STANDING (flex < 18°, step done).
 *
 * Warnings:
 *   - `valgus`                    — front knee caves toward midline (vs baseline knee width)
 *   - `trunk-forward`             — torso lean > 55°
 *   - `incomplete-walking-lunge`  — peak front-leg flex < MIN_REP_DEPTH on rep complete
...
import { WalkingLungeCalibration } from './calibration';
import type { WalkingLungeBaseline, WalkingLungeEngineCallbacks, WalkingLungeFrameMetrics, WalkingLungeRepState } from './types';
...
// Walking lunge is unilateral by design — back leg stays relatively straight,
// front leg flexes. We check that the front leg's peak is meaningfully larger
// than the back leg's, which catches "no real lunge" reps (both legs barely
// flex at all). Walking steps are often shorter than static lunges, so we
// use a lower threshold (15° instead of lunge's 20°).
const MIN_FRONT_BACK_GAP_DEG = 15;        // front-leg peak must exceed back-leg peak by this

export class WalkingLungeEngine {
  private callbacks: WalkingLungeEngineCallbacks;
  private calibration: WalkingLungeCalibration;
  private baseline: WalkingLungeBaseline | null = null;

  private repState: WalkingLungeRepState = 'STANDING';
  ...
  constructor(callbacks: WalkingLungeEngineCallbacks = {}) {
    this.callbacks = callbacks;
    this.calibration = new WalkingLungeCalibration();
  }
  ...
  update(landmarks: PoseLandmarks | null, now: number): void {
    ...
    if (!this.calibration.isConfirmed()) {
      ...
      if (calUpdate.state === 'confirmed') {
        ...
        if (this.baseline) {
          debugLog('WALKING-LUNGE', 'CALIB', 'CONFIRMED', {
  ...
  private processTrackingFrame(landmarks: PoseLandmarks, now: number): void {
    ...
    const frameMetrics: WalkingLungeFrameMetrics = {
    ...
    switch (this.repState) {
      case 'STANDING':
        if (this.smoothedFlexion > DESCEND_START_DEG) {
          ...
          debugLog('WALKING-LUNGE', 'STATE', 'STANDING → DESCENDING', {
            frontLeg: this.frontLeg,
      ...
      case 'DESCENDING': {
        ...
        if (delta < BOTTOM_STABILITY_DELTA) {
          ...
          if (this.stableBottomCount >= BOTTOM_STABILITY_FRAMES) {
            ...
            debugLog('WALKING-LUNGE', 'STATE', 'DESCENDING → AT_BOTTOM', { peak: +this.maxFlexionThisRep.toFixed(1) });
          }
        } else {
          this.stableBottomCount = 0;
        }
      ...
      case 'AT_BOTTOM': {
        ...
        if (deltaDown < -ASCENDING_DELTA_MIN || dropFromPeak >= ASCENT_FROM_PEAK_DEG) {
          ...
          debugLog('WALKING-LUNGE', 'STATE', 'AT_BOTTOM → ASCENDING', { peak: +this.maxFlexionThisRep.toFixed(1) });
        }
        break;
      }
  ...
  private completeRep(now: number): void {
    ...
    if (!validation.ok) {
      ...
      debugLog('WALKING-LUNGE', 'REJECT', 'Rep discarded', {
      ...
      if (validation.reason === 'too-shallow') {
        this.maybeEmitWarning('incomplete-walking-lunge' as WarningType, true, now);
      } else {
    ...
    debugLog('WALKING-LUNGE', 'REP', 'Rep complete', repPayload);
  ...
  private checkNoMovement(now: number): void {
    ...
    // post-rep decay tail (smoothedFlexion drifting from ~17° to 0°) doesn't
    ...
    if (
      ...
      debugLog('WALKING-LUNGE', 'WARN', 'not-moving', {
  ...
  // Posture gates
  // ----------------------------------------------------------
  private detectFrontKneeValgus(landmarks: PoseLandmarks, baseline: WalkingLungeBaseline): boolean {
  ...
  private maybeEmitWarning(type: WarningType, active: boolean, now: number): void {
    ...
    debugLog('WALKING-LUNGE', 'WARN', type);
  ...
  private checkPositionLost(haveValidFrame: boolean, now: number): void {
    ...
    debugLog('WALKING-LUNGE', 'WARN', 'position-lost', { lostMs: Math.round(lostMs) });
