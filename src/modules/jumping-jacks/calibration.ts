/**
 * Jumping Jacks calibration — 4 gates mirroring calf-raise's structure,
 * remapped to the CLOSED starting position (feet together, arms at sides):
 *   fullBodyVisible → shoulders+elbows+wrists+hips+knees+ankles visible
 *   feetWide        → feetClose (ankles within 0.7× shoulder width — together)
 *   armsOverhead    → armsAtSides (wrists BELOW shoulders + elbow flex < 25°)
 *   distanceOk      → body span in frame + shoulderWidth ≥ MIN_SHOULDER_WIDTH
 *                     (Fix X cal side)
 *
 * Baseline captures shoulderMidX (torso-swing reference) and shoulderMidY
 * (arm-openness reference for runtime composite signal).
 */
...
import type { JumpingJacksBaseline } from './types';
...
// Fix X (cal side): reject baselines with degenerately small shoulderWidth.
const MIN_SHOULDER_WIDTH = 0.08;

// Feet TOGETHER (CLOSED start position): ankles within 0.7× shoulder width.
const FEET_CLOSE_MAX_RATIO = 0.7;

// Arms relaxed at sides: both elbows nearly straight AND wrists below shoulders.
const ARMS_AT_SIDES_FLEX_MAX = 25;
...
export class JumpingJacksCalibration {
  private startedAt: number;
  private goodPostureStart = 0;
  private badPostureStart = 0;
  private confirmedBaseline: JumpingJacksBaseline | null = null;
  ...
  update(landmarks: PoseLandmarks | null, now: number): CalibrationUpdate {
    ...
    if (prevState !== this.state) {
      debugLog('JACKS', 'CALIB', `${prevState} → ${this.state}`, {
        gates: {
          fullBody: checks.fullBodyVisible,
          feetClose: checks.feetWide,
  ...
  private checkGates(landmarks: PoseLandmarks): {
    ...
    baselineCandidate: JumpingJacksBaseline | null;
  } {
    ...
    const feetClose = feetRatio <= FEET_CLOSE_MAX_RATIO;

    // Arms relaxed at sides: BOTH elbows nearly straight AND BOTH wrists below shoulders.
    const leftElbowFlex = elbowFlexionDeg(ls, le, lw);
    const rightElbowFlex = elbowFlexionDeg(rs, re, rw);
    const armsExtended = leftElbowFlex < ARMS_AT_SIDES_FLEX_MAX && rightElbowFlex < ARMS_AT_SIDES_FLEX_MAX;
    const shoulderY = (ls.y + rs.y) / 2;
    const wristsBelowShoulders = lw.y > shoulderY && rw.y > shoulderY;
    const armsAtSides = armsExtended && wristsBelowShoulders;
    ...
    const checks = {
      ...
      feetWide: feetClose,         // remap: "wide" slot → "close together"
      armsOverhead: armsAtSides,   // remap: "overhead" → "at sides"
      distanceOk,
    };
    ...
    const baseline: JumpingJacksBaseline = {
      shoulderMid,
      hipMid,
  ...
  getBaseline(): JumpingJacksBaseline | null { return this.confirmedBaseline; }
}
