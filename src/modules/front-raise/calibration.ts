/**
 * 2026-05-28 round 21: Front Raise re-architected to FRONT-camera. Calibration
 * clones lateral-raise's gates verbatim:
 *   fullBodyVisible → shoulders+elbows+wrists+hips+ankles visible
 *   feetWide        → feetStable (feet within 1.20× shoulder width)
 *   armsOverhead    → armsAtSides (BOTH arms at < 25° flexion — hanging at sides, DOWN start)
 *   distanceOk      → body span in frame, with shoulderWidth floor (Fix X)
 *
 * Bilateral: both arms must be at sides for cal to confirm. No side detection.
 */
...
import type { FrontRaiseBaseline } from './types';
...
const ARMS_AT_SIDES_ABDUCTION_MAX = 25; // both arms < 25° at calibration

const BODY_HEIGHT_MIN = 0.45;
const BODY_HEIGHT_MAX = 0.92;

// Fix X — narrow shoulderWidth at calibration → degenerate baseline.
...
export class FrontRaiseCalibration {
  private startedAt: number;
  private goodPostureStart = 0;
  private badPostureStart = 0;
  private confirmedBaseline: FrontRaiseBaseline | null = null;
  ...
  update(landmarks: PoseLandmarks | null, now: number): CalibrationUpdate {
    ...
    if (prevState !== this.state) {
      debugLog('FRONT', 'CALIB', `${prevState} → ${this.state}`, {
        gates: {
  ...
  private checkGates(landmarks: PoseLandmarks): {
    ...
    baselineCandidate: FrontRaiseBaseline | null;
  } {
    ...
    // Arms at sides (low flexion/abduction) — the DOWN start position.
    ...
    const baseline: FrontRaiseBaseline = {
      shoulderMid,
      hipMid,
  ...
  getBaseline(): FrontRaiseBaseline | null { return this.confirmedBaseline; }
}
