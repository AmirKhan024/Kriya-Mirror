/**
 * Deadlift calibration — side camera, person standing upright.
 *
...
import type { DeadliftBaseline } from './types';
import type { CalibrationBaseline, CalibrationState, CalibrationUpdate } from '@/modules/squat/types';
import { hipHingeDeg } from './geometry';
import { debugLog } from '@/lib/debug';

// 2026-05-25 round 5 (§3.5): instant confirm once all gates green.
...
export class DeadliftCalibration {
  private startedAt: number;
  private goodPostureStart = 0;
  private badPostureStart = 0;
  private confirmedBaseline: DeadliftBaseline | null = null;
  ...
  update(landmarks: PoseLandmarks | null, now: number): CalibrationUpdate {
    ...
    if (prevState !== this.state) {
      debugLog('DEADLIFT', 'CALIB', `${prevState} → ${this.state}`, {
        gates: {
  ...
  private checkGates(landmarks: PoseLandmarks): {
    ...
    baselineCandidate: DeadliftBaseline | null;
  } {
    // Pick the side with better shoulder+hip+ankle visibility (same approach as plank)
    ...
    const baseline: DeadliftBaseline = {
  ...
  getBaseline(): DeadliftBaseline | null { return this.confirmedBaseline; }
}

/** Adapt DeadliftBaseline to the shared CalibrationBaseline shape the play page reads. */
function toSquatBaseline(b: DeadliftBaseline): CalibrationBaseline {
  return {
