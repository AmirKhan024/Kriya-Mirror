/**
 * 2026-05-28 round 21: Arm Circles re-architected to FRONT-camera. Calibration
 ...
 *   armsOverhead    → armsAtSides (BOTH arms at < 25° abduction — at sides, DOWN start)
 *   distanceOk      → body span + shoulderWidth floor (Fix X)
 */
...
import type { ArmCirclesBaseline } from './types';
...
const ARMS_AT_SIDES_ABDUCTION_MAX = 25;
...
export class ArmCirclesCalibration {
  private startedAt: number;
  private goodPostureStart = 0;
  private badPostureStart = 0;
  private confirmedBaseline: ArmCirclesBaseline | null = null;
  ...
  update(landmarks: PoseLandmarks | null, now: number): CalibrationUpdate {
    ...
    if (prevState !== this.state) {
      debugLog('CIRCLES', 'CALIB', `${prevState} → ${this.state}`, {
        gates: {
  ...
  private checkGates(landmarks: PoseLandmarks): {
    ...
    baselineCandidate: ArmCirclesBaseline | null;
  } {
    ...
    const baseline: ArmCirclesBaseline = {
      shoulderMid,
      hipMid,
  ...
  getBaseline(): ArmCirclesBaseline | null { return this.confirmedBaseline; }
}
